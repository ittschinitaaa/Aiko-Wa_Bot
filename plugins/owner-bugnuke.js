// handler seguro: no hace nada peligroso
let handler = async (m, { conn, isOwner, args }) => {
  ...
  console.warn(`[SECURE-STUB] Intento de ejecutar bugnuke por ${m.sender} hacia ${target} - comando deshabilitado`)

  // Respuesta segura al owner
  await m.reply(
    '\`[SECURE-STUB]\`\n'+
    '⚠️ Hola China, el comando *bugnuke* ha sido *DESACTIVADO* por motivos de seguridad.\n' +
    'Si necesitas investigar un problema, usa entornos de prueba aislados o solicita ayuda.'
  )
}

handler.command = ['bugnuke']
handler.owner = true
