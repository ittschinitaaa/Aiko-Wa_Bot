  ...
  try {
    ...
    switch (command) {
      ...
      case 'unwarn': {
        ...
        let user = global.db.data.users[who]
        if (!user || !user.warn) return m.reply(`ꕥ El usuario tiene 0 advertencias.`)

        user.warn -= 1
        await m.reply(`🍯 A *@${who.split`@`[0]}* se le quitó una advertencia.\n*Advertencias ${user.warn}/3*`, null, { mentions: [who] })
        break
      }
